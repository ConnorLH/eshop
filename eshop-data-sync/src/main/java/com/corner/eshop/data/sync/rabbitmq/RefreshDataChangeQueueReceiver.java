...
public class RefreshDataChangeQueueReceiver {
    ...
    private void processBrandChangeMessage(JSONObject jsonObject){
        ...
        if(StringUtils.equals("add",eventType) || StringUtils.equals("update",eventType)){
            ...
            JSONObject parseObject = JSONObject.parseObject(brandJson);
            log.info("Process add or update brand message, Obtain brand change data:{}",brandJson);
            redisTemplate.opsForValue().set("brand:"+id,brandJson);
