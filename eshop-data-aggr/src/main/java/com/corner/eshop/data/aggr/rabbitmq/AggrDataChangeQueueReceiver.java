...
public class AggrDataChangeQueueReceiver {
    ...
    private void processProductDimDataChange(JSONObject messageJSONObject) {
        Long id = messageJSONObject.getLong("id");
        // 优化为批量查询商品信息
        List productInfos = redisTemplate.opsForValue().multiGet(Arrays.asList("product:" + id, "product:property:" + id, "product:specification:" + id));
        String productDataJSON = (String)productInfos.get(0);
        ...
        if(productDataJSON != null && !"".equals(productDataJSON)) {
            ...
            String productPropertyDataJSON = (String)productInfos.get(1);
            ...
            String productSpecificationDataJSON = (String)productInfos.get(2);
